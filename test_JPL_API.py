...
# e.g., 4:1 NRHO = 7.375 days = 1.664 TU
tof = 1.664
...
# Plot the orbits
ax.plot(pos[:,0]*LU - (1-mu)*LU, pos[:,1]*LU, pos[:,2]*LU, label="4:1 resonance")

# line, = ax.plot([], [], [], lw=2, label="9:2 resonance")

# e.g., rp=2000km NRHO, 5.9 days
tof = 5.9 * 24*60*60 / TU
row = df.loc[(df.iloc[:,7]-tof).abs().argsort()[0]]
tof = row[7]
s0 = row[0:6]
pos, vel = propagate_cr3bp(s0, mu, tof, step=steps)
ax.plot(pos[:,0]*LU - (1-mu)*LU, pos[:,1]*LU, pos[:,2]*LU, label="rp = 2000km")
...
pos, vel = propagate_cr3bp(s0, mu, tof, step=101)
tvec = np.linspace(0, tof, 101)


# create the animation
# ani = FuncAnimation(fig, update, frames=len(tvec), interval=5, fargs=(pos[:,0]*LU - (1-mu)*LU, pos[:,1]*LU, pos[:,2]*LU))
