...
def plot_inv_manifold(fig, ax, mu, s0, period, eps='fix', arc_num=10, t_prop=100, d=2):
    ...
    for i in range(arc_num):
        ...
        (pos_u1, vel_u1) = propagate_cr3bp(xui1, mu, t_prop, step=200)
        (pos_u2, vel_u2) = propagate_cr3bp(xui2, mu, t_prop, step=200)
        (pos_s1, vel_s1) = propagate_cr3bp(xsi1, mu, -t_prop, step=200)
        (pos_s2, vel_s2) = propagate_cr3bp(xsi2, mu, -t_prop, step=200)
        ...
        if d == 3:
            ...
            # ax.plot(pos_u1[:,0], pos_u1[:,1], pos_u1[:,2], color='brown', linewidth=1)
            ax.plot(pos_u2[:, 0], pos_u2[:, 1], color='red', linewidth=1)
            ax.plot(pos_s1[:, 0], pos_s1[:, 1], color='lime', linewidth=1)
            # ax.plot(pos_s2[:,0], pos_s2[:,1], pos_s2[:,2], color='green', linewidth=1)
            # ax.scatter(pos_s1[0,0], pos_s1[0,1], pos_s1[0,2], color='lime', marker="x")
            # ax.scatter(pos_s2[0,0], pos_s2[0,1], pos_s2[0,2], color='green', marker="x")
